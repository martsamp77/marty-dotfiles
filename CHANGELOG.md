...
- **[`.chezmoi/templates/marty-powershell.ps1.tmpl`](.chezmoi/templates/marty-powershell.ps1.tmpl)** — When Starship is enabled: **transient prompt** (**`Invoke-Starship-TransientFunction`** + **`Enable-TransientPrompt`**) per [advanced config](https://starship.rs/advanced-config/) (wrapped in **`try`/`catch`** for older Starship). ZSH-like directory listings: **`Show-MartySortedChildItems`** (directories first, then files); **`cd`** uses it after navigation; **`ls`** / **`dir`** replace aliases and forward common **`Get-ChildItem`** parameters; **`gci`** / **`Get-ChildItem`** unchanged for default output. PSReadLine **syntax + completion menu** colors (subtle ANSI) when supported.
