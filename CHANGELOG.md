...
- **[`.chezmoitemplates/marty-powershell.ps1.tmpl`](.chezmoitemplates/marty-powershell.ps1.tmpl)** / **[`.chezmoi/templates/marty-powershell.ps1.tmpl`](.chezmoi/templates/marty-powershell.ps1.tmpl)** — **`ls`** / **`dir`**: **`Show-MartySortedChildItems`** (directories then files with details). **`cd`**: after navigation, **subfolders only** via **`Format-Wide`** (horizontal columns). When Starship is enabled: **transient prompt** per [advanced config](https://starship.rs/advanced-config/). **`gci`** / **`Get-ChildItem`** unchanged for default output. PSReadLine **syntax + completion menu** colors when supported.
