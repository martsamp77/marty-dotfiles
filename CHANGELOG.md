...
- **[`.chezmoitemplates/marty-powershell.ps1.tmpl`](.chezmoitemplates/marty-powershell.ps1.tmpl)** / **[`.chezmoi/templates/marty-powershell.ps1.tmpl`](.chezmoi/templates/marty-powershell.ps1.tmpl)** — **`ls`** / **`dir`**: subfolder **names only** (one per line, no details table), zsh-like list view; **`Get-ChildItem -File`** still uses native listing. **`cd`** after navigation unchanged: **`Show-MartySortedChildItems`** (directories then files with details). When Starship is enabled: **transient prompt** per [advanced config](https://starship.rs/advanced-config/). **`gci`** / **`Get-ChildItem`** unchanged for default output. PSReadLine **syntax + completion menu** colors when supported.
